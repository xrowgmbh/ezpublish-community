...
$generator = new ezpBinTravisHttpConfGenerator(
    ...
    array(
        ...
        'env' => null,// set to 'dev' or 'prod' later depending on provided template name
        'proxy' => '127.0.0.1'
    )
);
...
if ( $options['env'] === null )
{
    $options['env'] = strpos( $templateFile, '.dev.' ) !== false ? 'dev' : 'prod';
}
...
Example use:
  \$ sudo php bin/.travis/generate_vhost.php --basedir=/var/www --env=behat doc/conf/httpd/apache2/vhost.fastcgi.prod.template /etc/apache2/sites-available/behat
